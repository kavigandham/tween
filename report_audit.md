...
> **Addendum 2 (6853735 audit → fixed in 0bd87ed/84292c1):** the maps-preference audit verdicts were clean on storage/routing/settings, but it caught the sibling pattern on the feature itself — **SpotDetailCard's action-tile Directions and primary Directions button bypassed the preference** (only the fallback secondary button was converted), and the Apple branch opened a search PIN while Google opened DIRECTIONS. FIXED: every place-card Directions control now routes through one preference dispatcher (Apple keeps place identity via the map item; both branches open driving directions). Also FIXED from its straggler passes: `hasLiveMeetup`'s draft branch now ages out on the 24 h TTL (a stale never-sent draft could suppress the host cold-launch reset forever); ResultRows a11y hint no longer claims "Apple Maps"; stale route-call arithmetic comment. Dead code it re-confirmed (insertBubble, legacy makeImage, preview-only rows) was DELETED in `84292c1`. Remaining new MINORs triaged into the carried queue: `setActive(true)` outside the sendKey gate, cancelled sendTask clearing `isSending`, `decodeAndCache` mixed store keys, `retryBlankRenderIfNeeded` teardown, preference-dispatch test gap, SettingsSheet/gear UI-test gap.
